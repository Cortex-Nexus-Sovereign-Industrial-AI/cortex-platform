 ...
 * await shopify.createProduct({ title, price, inventory })
 */

const STORE = process.env.SHOPIFY_STORE_DOMAIN || 'cortex-intelligence-nexus.myshopify.com';
const TOKEN = process.env.SHOPIFY_ADMIN_TOKEN; // shpat_ or atkn_
...
export const shopify = {
  ...
  createProduct: ({ title, description, price, sku, inventory = 10 }) =>
    gql(`
      ...
      input: {
        ...
        variants: [{ price, sku, inventoryQuantities: { availableQuantity: inventory, locationId: undefined } }]
      }
    }),
  ...
  updateInventory: (inventoryItemId, locationId, available) =>
    gql(`
      mutation { inventoryAdjustQuantities(input: { reason: "correction", name: "available", changes: [{ inventoryItemId: "${inventoryItemId}", locationId: "${locationId}", delta: ${available} }] }) { userErrors { message } } }
    `),
