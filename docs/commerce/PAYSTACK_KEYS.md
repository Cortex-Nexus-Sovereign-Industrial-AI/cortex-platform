...
2. [ ] `PAYSTACK_SECRET_KEY` set on Netlify (Test secret from same account)
3. [ ] `PAYSTACK_PUBLIC_KEY` set on Netlify = the pk_test above (optional but clean)
4. [ ] Webhook URL saved in Paystack (Test mode)
5. [ ] One test charge → function logs show 200
